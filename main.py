from fastapi import FastAPI, Request, Header, HTTPException, Query, BackgroundTasks, Depends, UploadFile, File
...
async def telegram_webhook(request: Request):
    ...
    if "photo" in message and isinstance(message["photo"], list) and message["photo"]:
        try:
            ...
            if file_id and token:
                ...
                gf_res = requests.get(gf_url, params={"file_id": file_id}, timeout=12)
    ...
    if text_msg:
        ...
        if is_off:
            ...
            if media_id_logged:
                log_received_message(
                    linewalker=linewalker, 
                    message=f"📷 [image received] {PUBLIC_WEBHOOK_URL}/media/thumb?id={media_id_logged}", 
                    user=user_first,
        ...
        if media_id_logged:
            log_received_message(
                linewalker=linewalker, 
                message=f"[caption] {caption} {PUBLIC_WEBHOOK_URL}/media/thumb?id={media_id_logged}", 
                user=user_first,
                media_id=media_id_logged,
                media_type=media_type_logged
            )
