...
if not TELEGRAM_WEBHOOK_SECRET:
    raise RuntimeError("TELEGRAM_WEBHOOK_SECRET must be set for /webhook verification")
...
@limiter.limit("5/minute")
def admin_set_webhook(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
...
@limiter.limit("10/minute")
def admin_get_webhook_info(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
        raise HTTPException(status_code=400, detail="BOT_TOKEN is not set")
    try:
...
@limiter.limit("5/minute")
def clear_old_logs(days: int = Query(7, ge=1, le=365),
                   unsealed: bool = Depends(require_unlocked),
                   auth=Depends(verify_api_key),
                   sig=Depends(verify_signed_request)):
...
@limiter.limit("5/minute")
def backup_and_clear(days: int = Query(7, ge=1, le=365),
                     unsealed: bool = Depends(require_unlocked),
                     auth=Depends(verify_api_key),
                     sig=Depends(verify_signed_request)):
    try:
