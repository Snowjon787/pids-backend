...
# ---------- Telegram session + timeout ----------
TELEGRAM_TIMEOUT = int(os.getenv("TELEGRAM_TIMEOUT", "12"))  # seconds
...
TG_SESSION.mount("http://",  HTTPAdapter(max_retries=tg_retries))
...
def reset_all_roles(auth=Depends(verify_api_key)):
    result = {}
    try:
        with engine.begin() as conn:
...
@app.post("/send_alert")
@limiter.limit("20/minute")
def send_alert(payload: AlertPayload, background_tasks: BackgroundTasks):
...
@limiter.limit("5/minute")
def clear_old_logs(days: int = Query(7, ge=1, le=365), auth=Depends(verify_api_key)):
...
@limiter.limit("5/minute")
def backup_and_clear(days: int = Query(7, ge=1, le=365), auth=Depends(verify_api_key)):
    try:
        with engine.begin() as conn:
            df_sent = pd.read_sql(text("SELECT * FROM sent_logs ORDER BY id DESC"), conn)
            df_received = pd.read_sql(text("SELECT * FROM received_messages ORDER BY id DESC"), conn)
...
@app.get("/set_webhook")
@limiter.limit("5/minute")
def set_telegram_webhook(auth=Depends(verify_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
...
def test_webhook(auth=Depends(verify_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
...
def refresh_backend(req: RefreshRequest, auth=Depends(verify_api_key)):
    ...
    if req.reset_webhook:
        try:
            result["webhook"] = set_telegram_webhook(auth)  # reuse auth
