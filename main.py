...
from fastapi.middleware.cors import CORSMiddleware
    # NOTE: we will attach session-expiry headers in a custom middleware below
...
# Expiry hint knobs for Bearer sessions (new)
SESSION_REFRESH_HINT_SECONDS = int(os.getenv("SESSION_REFRESH_HINT_SECONDS", "900"))  # 15 min
SESSION_FORCE_REFRESH_SECONDS = int(os.getenv("SESSION_FORCE_REFRESH_SECONDS", "300"))  # 5 min
...
def init_db_pg():
    ...
    CREATE INDEX IF NOT EXISTS ix_refresh_tokens_exp ON refresh_tokens(expires_at);
    """
    with engine.begin() as conn:
...
# ==============================
# 30 days access by default (env override allowed)
SESSION_TTL_HOURS = int(os.getenv("SESSION_TTL_HOURS", "720"))  # 30 days
REFRESH_TTL_DAYS = int(os.getenv("REFRESH_TTL_DAYS", "60"))     # remember-me 60 days default

def _now_utc():
    return datetime.utcnow()

def _session_pack(payload: Dict[str, Any]) -> str:
    p = dict(payload)
    p["iat"] = int(_now_utc().timestamp())
    p["exp"] = int((_now_utc() + timedelta(hours=SESSION_TTL_HOURS)).timestamp())
    blob = json.dumps(p, separators=(",", ":")).encode()
    return FERNET.encrypt(blob).decode()

def _session_unpack(token: str) -> Dict[str, Any]:
    try:
        data = FERNET.decrypt(token.encode())
        obj = json.loads(data.decode())
        if int(obj.get("exp", 0)) < int(_now_utc().timestamp()):
...
# <<< NEW: Expiry advice middleware >>>
class ExpiryAdviceMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        response = await call_next(request)
        ttl = getattr(request.state, "session_ttl", None)
        exp_iso = getattr(request.state, "session_exp_at", None)
        if ttl is not None:
            response.headers["X-Session-Expires-In"] = str(int(ttl))
        if exp_iso is not None:
            response.headers["X-Session-Expires-At"] = exp_iso
        # Hints for the frontend to proactively refresh
        if ttl is not None:
            if ttl <= SESSION_FORCE_REFRESH_SECONDS:
                response.headers["X-Session-Action"] = "refresh-now"
            elif ttl <= SESSION_REFRESH_HINT_SECONDS:
                response.headers["X-Session-Action"] = "refresh-soon"
        return response

app.add_middleware(ExpiryAdviceMiddleware)

def _stash_session_advice(request: Request, obj: Dict[str, Any]):
    # helper to stash ttl/exp onto request for middleware headers
    now = int(_now_utc().timestamp())
    ttl = max(0, int(obj.get("exp", 0)) - now)
    request.state.session_ttl = ttl
    # present expiry as IST ISO for convenience
    try:
        exp_dt = datetime.fromtimestamp(int(obj.get("exp", 0)), pytz.UTC).astimezone(IST)
        request.state.session_exp_at = exp_dt.strftime("%Y-%m-%d %H:%M:%S %Z")
    except Exception:
        request.state.session_exp_at = None
...
def verify_bearer_or_api_key(
    ...
    # Accept either API key OR Bearer session
    if x_api_key:
    ...
    if tok:
        obj = _session_unpack(tok)  # raises if bad/expired
        _stash_session_advice(request, obj)
        return {"auth": "bearer"}

    record_fail("no_auth")
    raise HTTPException(401, "Missing Authorization (Bearer) or X-API-KEY")
...
def verify_signed_or_bearer(
    ...
    tok = _read_bearer(authorization)
    if tok:
        obj = _session_unpack(tok)  # raises if invalid/expired
        _stash_session_advice(request, obj)
        return {"auth": "bearer"}
    # Fallback to strict signed flow
    if not all([x_client_id, x_api_key, x_ts, x_nonce, x_sig]):
        record_fail("signed_missing")
        raise HTTPException(401, "Signed headers missing (or provide Bearer token)")
...
# ==============================
# Auth: Login / Refresh / Logout / Extend (+ Introspect)
# ==============================
...
def _refresh_pack(jti: str, payload: Dict[str, Any]) -> str:
    p = dict(payload)
    p["jti"] = jti
    p["iat"] = int(_now_utc().timestamp())
    p["exp"] = int((_now_utc() + timedelta(days=REFRESH_TTL_DAYS)).timestamp())
    return FERNET.encrypt(json.dumps(p, separators=(",", ":")).encode()).decode()

def _refresh_unpack(token: str) -> Dict[str, Any]:
    try:
        obj = json.loads(FERNET.decrypt(token.encode()).decode())
        if int(obj.get("exp", 0)) < int(_now_utc().timestamp()):
...
def _refresh_store(jti: str, subject: str, device_id: str, user_label: str, expires_at: datetime):
    db_exec("""
      INSERT INTO refresh_tokens (jti, subject, device_id, user_label, expires_at, revoked)
      VALUES (:j, :s, :d, :u, :e, FALSE)
      ON CONFLICT (jti) DO NOTHING
    """, {"j": jti, "s": subject, "d": device_id, "u": user_label, "e": expires_at.strftime("%Y-%m-%d %H:%M:%S")})

def _refresh_get(jti: str) -> Optional[Dict[str, Any]]:
    return db_query_one("SELECT * FROM refresh_tokens WHERE jti=:j", {"j": jti})

def _refresh_revoke(jti: str):
    db_exec("UPDATE refresh_tokens SET revoked=TRUE WHERE jti=:j", {"j": jti})

@app.post("/auth/login", response_model=LoginResp)
def auth_login(req: LoginReq):
    if req.api_key != API_KEY:
        record_fail("login_bad_key")
        raise HTTPException(403, "Unauthorized")
    payload = {"sub": "desktop-app", "device_id": req.device_id or "", "user": req.user or ""}
    access = _session_pack(payload)
    access_exp = json.loads(FERNET.decrypt(access.encode()).decode())["exp"]
    jti = _new_jti()
    refresh = _refresh_pack(jti, payload)
    refresh_exp = json.loads(FERNET.decrypt(refresh.encode()).decode())["exp"]
    _refresh_store(
        jti=jti,
        subject=payload["sub"],
    ...
    return LoginResp(
        ...
        expires_at=datetime.fromtimestamp(access_exp, pytz.UTC).astimezone(IST).strftime("%Y-%m-%d %H:%M:%S %Z"),
        refresh_token=refresh,
        refresh_expires_at=datetime.fromtimestamp(refresh_exp, pytz.UTC).astimezone(IST).strftime("%Y-%m-%d %H:%M:%S %Z"),
    )

@app.post("/auth/refresh", response_model=RefreshResp)
def auth_refresh(req: RefreshReq):
    obj = _refresh_unpack(req.refresh_token)
    jti = obj.get("jti") or ""
    rec = _refresh_get(jti)
    if not rec or rec.get("revoked"):
        raise HTTPException(401, "Refresh revoked or unknown")
    payload = {"sub": obj.get("sub"), "device_id": obj.get("device_id", ""), "user": obj.get("user", "")}
    access = _session_pack(payload)
    access_exp = json.loads(FERNET.decrypt(access.encode()).decode())["exp"]
    return RefreshResp(
        access_token=access,
...
@app.post("/auth/extend", response_model=RefreshResp)
def auth_extend(auth=Depends(verify_bearer_or_api_key)):
    # Mint a fresh access token for a valid session (sliding behavior)
    access = _session_pack({"sub": "desktop-app"})
    access_exp = json.loads(FERNET.decrypt(access.encode()).decode())["exp"]
    return RefreshResp(
        access_token=access,
        expires_in_seconds=SESSION_TTL_HOURS * 3600,
...
# <<< NEW: Introspect current Bearer and return remaining TTL >>>
@app.get("/auth/introspect")
def auth_introspect(authorization: Optional[str] = Header(None)):
    tok = _read_bearer(authorization)
    if not tok:
        raise HTTPException(401, "Missing Bearer token")
    obj = _session_unpack(tok)
    now = int(_now_utc().timestamp())
    ttl = max(0, int(obj.get("exp", 0)) - now)
    return {
        "active": True,
...
def upload_master_csv2(
    ...
    auth=Depends(verify_bearer_or_api_key),
    sig=Depends(verify_signed_or_bearer),
):
...
@app.get("/master/info")
def master_info(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
...
# ---------------- Section & conversion utilities (public) ----------------
...
def update_token(request: Request, data: TokenUpdateRequest,
                 unsealed: bool = Depends(require_unlocked),
                 auth=Depends(verify_bearer_or_api_key), sig=Depends(verify_signed_or_bearer)):
...
def view_linewalkers(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
...
def view_supervisors(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
...
@app.get("/view_npv")
def view_npv(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    return load_npv()
...
@app.get("/get_logs")
def get_logs(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
...
def get_media_logs(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
        df = _df_from_query("""
...
def get_received_logs(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
        cols = _select_list(
            "received_messages", 
...
def get_duty_logs(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
...
def download_logs(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
...
def download_all_logs(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
        df_sent = _df_from_query("""
            SELECT id, Date, Time, OD, CH, Section, Linewalker, Supervisor, NPV 
            FROM sent_logs ORDER BY id DESC
...
def get_scatter_chart(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
        with engine.begin() as conn:
...
def get_group_bar_chart(group_by: str = Query("section", **_QUERY_REGEX_KW),
                        unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    try:
        with engine.begin() as conn:
            df = pd.read_sql(text("SELECT * FROM sent_logs"), conn)
            if df.empty:
...
@limiter.limit("5/minute")
def admin_set_webhook(request: Request, unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
    ...
    try:
        # Use POST to Telegram for setWebhook
        res = requests.post(
...
@limiter.limit("10/minute")
def admin_get_webhook_info(request: Request, unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
        raise HTTPException(status_code=400, detail="BOT_TOKEN is not set")
    try:
...
def _set_webhook_now() -> Dict[str, Any]:
    ...
    if TELEGRAM_WEBHOOK_SECRET:
        params["secret_token"] = TELEGRAM_WEBHOOK_SECRET
    try:
        r = requests.post(f"https://api.telegram.org/bot{token}/setWebhook", data=params, timeout=8)
        return r.json()
    except requests.RequestException as e:
        logger.exception({"msg": "set_webhook failed", "error": str(e)})
        raise HTTPException(status_code=500, detail="Internal error")
...
def migrate_json_to_db(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    result = {}
    try:
...
def clear_old_logs(request: Request, days: int = Query(7, ge=1, le=365),
    ...
    try:
        ...
        try:
            ...
            logger.warning({"msg": "VACUUM skipped/failed", "error": str(e)})
...
@app.post("/admin/lock")
def admin_lock(auth=Depends(verify_bearer_or_api_key)):
    _set_locked(True)
...
# ---------------- Aliases (protected) ----------------
@app.get("/set_webhook")
def set_webhook_alias(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
    return _set_webhook_now()

@app.get("/test_webhook")
def test_webhook_alias(unsealed: bool = Depends(require_unlocked), auth=Depends(verify_bearer_or_api_key)):
