...
def migrate_json_to_db(auth=Depends(verify_api_key)):
    result = {}
    try:
...
@limiter.limit("20/minute")
def edit_linewalkers(request: Request, data: List[LineWalkerItem], auth=Depends(verify_api_key)):
...
@limiter.limit("20/minute")
def edit_supervisors(request: Request, data: List[SupervisorItem], auth=Depends(verify_api_key)):
...
@app.post("/edit_npv")
@limiter.limit("20/minute")
def edit_npv(request: Request, data: List[NPVItem], auth=Depends(verify_api_key)):
...
def reset_all_roles(request: Request, auth=Depends(verify_api_key)):
    result = {}
    try:
        with engine.begin() as conn:
...
@limiter.limit("5/minute")
def clear_old_logs(request: Request, days: int = Query(7, ge=1, le=365), auth=Depends(verify_api_key)):
...
@limiter.limit("5/minute")
def backup_and_clear(request: Request, days: int = Query(7, ge=1, le=365), auth=Depends(verify_api_key)):
    try:
        with engine.begin() as conn:
            df_sent = pd.read_sql(text("SELECT * FROM sent_logs ORDER BY id DESC"), conn)
            df_received = pd.read_sql(text("SELECT * FROM received_messages ORDER BY id DESC"), conn)
...
@app.get("/set_webhook")
@limiter.limit("5/minute")
def set_telegram_webhook(request: Request, auth=Depends(verify_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
...
def test_webhook(request: Request, auth=Depends(verify_api_key)):
    token = get_setting("BOT_TOKEN")
    if not token:
...
def refresh_backend(req: RefreshRequest, auth=Depends(verify_api_key)):
    ...
    if req.migrate_from_json:
        result["migration"] = migrate_json_to_db(auth)  # reuse auth
    if req.reset_webhook:
        try:
            result["webhook"] = set_telegram_webhook(Request, auth)  # will be ignored; kept for API symmetry
