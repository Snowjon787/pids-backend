...
DEFAULT_SETTINGS = {
    "BOT_TOKEN": "",
    "CHAT_IDS": {},  # ✅ key: label/name, value: chat ID
    "ALERT_SOUND": True,
    "TIMEOUT": 10
}
...
def load_settings():
    ...
    if os.path.exists(SETTINGS_FILE):
        ...
        # Merge with defaults (preserve existing keys)
        settings = {**DEFAULT_SETTINGS, **loaded}

        # Re-save if new keys were added
        if settings != loaded:
...
class TokenData(BaseModel):
    token: str
    chat_ids: Dict[str, str]  # ✅ Expect dict of labels to chat IDs
...
def update_token(data: TokenData, auth=Depends(verify_api_key)):
    try:
        ...
        return {"status": "success", "message": "Bot token and chat IDs updated"}
...
@app.get("/reload_sections")
def reload_sections():
    section_data = load_section_data()
    return {"status": "reloaded", "sections": list(section_data.keys())}
...
def view_logs():
    try:
        ...
        if platform.system() == "Windows":
          ...
          subprocess.call(["open", EXCEL_EXPORT_FILE])  # Mac
    # subprocess.call(["xdg-open", EXCEL_EXPORT_FILE])  # Linux
