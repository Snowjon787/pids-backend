...
def send_alert(payload: AlertPayload):
    ...
    # Select sender info by time window
    ...
    if not chat_id:
        return JSONResponse(
            ...
            content={
                "status": "error",
                "detail": f"No group available for CH {payload.ch} at this time"
            }
        )
    ...
    if not settings.get("BOT_TOKEN"):
        return JSONResponse(
            ...
            content={
                "status": "error",
                "detail": "BOT_TOKEN is missing from settings"
            }
        )
    ...
    try:
        ...
        print("❌ Send alert exception:", str(e))
        return JSONResponse(
            status_code=500,
            content={
                "status": "error",
