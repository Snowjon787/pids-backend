...
# === Load Group Mappings ===
def load_group_mappings(force_reload=False):
    global group_mappings
    if force_reload or not group_mappings:
...
group_mappings = load_group_mappings(force_reload=True)
...
def update_token(data: TokenUpdateRequest, auth=Depends(verify_api_key)):
    ...
    try:
        if not data.token.strip():
            return JSONResponse(status_code=400, content={"status": "error", "detail": "Token cannot be empty"})
...
def update_group_mappings(payload: GroupMappingPayload, auth=Depends(verify_api_key)):
    try:
        ...
        reloaded = load_group_mappings(force_reload=True)
...
# === Chat ID Selection Function ===
def get_chat_id_by_ch_and_time(ch):
    group_mappings = load_group_mappings(force_reload=True)
    now = datetime.now(pytz.timezone("Asia/Kolkata")).replace(microsecond=0).time()
...
def set_webhook():
    try:
        webhook_url = "https://pids-backend.onrender.com/webhook"  # ✅ full path
