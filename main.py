...
def load_settings():
    ...
    if os.path.exists(SETTINGS_FILE):
        ...
        # If merged settings differ (e.g., added new default keys), save them
        if settings != loaded:
...
# ✅ Model to accept bot token and group mappings
class TokenData(BaseModel):
    token: str
    group_mappings: List[Dict[str, str]]  # Each item: start_ch, end_ch, group_name, chat_id

# ✅ Dummy API key verifier (you should define this somewhere)
def verify_api_key(x_api_key: str = Header(...)):
    if x_api_key != "Yj@mb51":
        raise HTTPException(status_code=403, detail="Unauthorized")

# ✅ API to update bot token and group mappings
...
def update_token(data: TokenData, auth=Depends(verify_api_key)):
    ...
    try:
        ...
        # ✅ Save group mappings to group_mapping.json
