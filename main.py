...
def load_group_mappings() -> List[Dict[str, Any]]:
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("SELECT start_ch, end_ch, group_name, chat_id, time_window FROM group_mappings")
        return [{"start_ch": r[0], "end_ch": r[1], "group_name": r[2], "chat_id": r[3], "time_window": r[4]} for r in cur.fetchall()]
...
def update_group_mappings(mappings: List[Dict[str, Any]]) -> None:
    with get_conn() as conn:
        ...
        cur.executemany("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, [(m["start_ch"], m["end_ch"], m["group_name"], m["chat_id"], m["time_window"]) for m in mappings])
...
# ===================== Reset Roles =====================
@app.post("/reset_all_roles", dependencies=[Depends(verify_api_key)])
def reset_all_roles():
    result = {}
    try:
        with get_conn() as conn:
