...
def init_db_pg():
    ...
    -- Logs (NOTE: using TitleCase columns to match older data)
    ...
    -- Received text/caption messages, now with media linkage
    CREATE TABLE IF NOT EXISTS received_messages (
        id SERIAL PRIMARY KEY,
        Timestamp TEXT, Linewalker TEXT, Message TEXT, Sender TEXT,
        MediaId INTEGER, MediaType TEXT, MediaThumb TEXT
    );
...
def download_logs():
    try:
        ...
        data = _df_to_excel_bytes({
            "Sent Logs": df_sent, "Received Logs": df_received, "Duty Status": df_duty, "Received Media": df_media
        })
...
def _df_from_query(sql: str, params: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    with engine.begin() as conn:
        ...
        rows = res.fetchall()
        cols = res.keys()
    # Return an empty DF with columns when no rows, to keep the UI happy
...
def get_received_logs():
    try:
        ...
        # Return JSON (not plain text) so the frontend can show the real reason
        return JSONResponse(status_code=500, content={"detail": "Internal error", "why": str(e)})

@app.get("/get_duty_logs")
def get_duty_logs():
    try:
        df = _df_from_query("SELECT * FROM duty_status ORDER BY id DESC")
        return df.to_dict(orient="records")
    except Exception as e:
...
def download_all_logs():
    try:
        df_sent = _df_from_query("""
            ...
            ORDER BY id DESC
        """)
        df_received = _df_from_query("""
            SELECT id, Timestamp, Linewalker, Message, Sender, MediaId, MediaType, MediaThumb
            FROM received_messages
            ORDER BY id DESC
        """)
        df_duty = _df_from_query("SELECT * FROM duty_status ORDER BY id DESC")
