...
def admin_get_webhook_info(request: Request, auth=Depends(verify_api_key)):
    ...
    if not token:
        raise HTTPException(status_code=400, detail="BOT_TOKEN is not set")
    try:
        res = SESSION.get(f"https://api.telegram.org/bot{token}/getWebhookInfo", timeout=REQUEST_TIMEOUT)
        return res.json()
    except requests.RequestException as e:
...
# Back-compat routes
@app.get("/set_webhook")
def set_webhook_alias(auth=Depends(verify_api_key)):
    return admin_set_webhook(Request, auth)  # type: ignore

@app.get("/test_webhook")
def test_webhook_alias(auth=Depends(verify_api_key)):
