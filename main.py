...
from fastapi import FastAPI, Request, Header, HTTPException, Query, BackgroundTasks, Depends
...
# Render can still hand out postgres:// — upgrade to psycopg driver
...
# Engine tuned for serverless-ish platforms
engine = create_engine(
    DATABASE_URL,
    pool_pre_ping=True,
    pool_size=int(os.getenv("DB_POOL_SIZE", "5")),
    max_overflow=int(os.getenv("DB_MAX_OVERFLOW", "10")),
    pool_recycle=1800,  # recycle stale conns after 30 min
    future=True,
)
...
def init_db_pg():
    ...
    -- Logs (NOTE: TitleCase columns to match older data)
    ...
    -- Received text/caption messages (we'll add newer columns below if missing)
    CREATE TABLE IF NOT EXISTS received_messages (
        id SERIAL PRIMARY KEY,
        Timestamp TEXT, Linewalker TEXT, Message TEXT, Sender TEXT
        -- MediaId / MediaType / MediaThumb may be added by ensure_schema()
    );
    CREATE INDEX IF NOT EXISTS ix_received_time ON received_messages(Timestamp);

    CREATE TABLE IF NOT EXISTS duty_status (
        id SERIAL PRIMARY KEY,
        Timestamp TEXT, Linewalker TEXT
        -- Duty_on / Duty_off may be added by ensure_schema()
    );
...
def ensure_schema():
    """
    Adds newer columns to legacy tables without dropping data.
    Prevents SELECT errors like 'column "MediaId" does not exist'.
    """
    alters = []

    # received_messages optional media columns
...
# Run DDL and quick migrations on import
init_db_pg()
ensure_schema()
...
MAX_CSV_BYTES = int(os.getenv("MAX_CSV_BYTES", "10485760"))  # 10 MiB default
...
# (the rest of your original backend code continues here: reset_all_roles, send_alert, logs, analytics, webhook, etc.)
# ===================== Reset Roles =====================
...
# --- helper to read a query into a DataFrame without pd.read_sql ---
...
# --- utils to build SELECT lists that work even if columns don't exist yet ---
        ...
        with engine.begin() as conn:
            return conn.execute(
                text("""
                    ...
                    LIMIT 1
                """),
                {"t": table.lower(), "c": column}
            ).scalar() is not None
    except Exception:
        # If metadata probe fails, assume column exists so we don't mask other errors.
        return True
...
def download_logs():
    try:
        ...
        """)
        # received_messages may be missing Media* cols on older DBs
        ...
        df_received = _df_from_query(f"""
            SELECT {recv_cols}
            FROM received_messages ORDER BY id DESC
        """)
        # duty_status may be missing Duty_on/Duty_off on older DBs
        duty_cols = _select_list("duty_status", ["id","Timestamp","Linewalker","Duty_on","Duty_off"])
...
def get_duty_logs():
    try:
        cols = _select_list("duty_status", ["id","Timestamp","Linewalker","Duty_on","Duty_off"])
...
def _download_with_cap(url: str, timeout: int = TELEGRAM_TIMEOUT) -> bytes:
    with TG_SESSION.get(url, stream=True, timeout=timeout) as r:
        ...
        return buf.read() if hasattr(buf, "read") else buf.getvalue()
...
def log_received_message(linewalker: str, message: str, user: str,
                         media_id: Optional[int] = None, media_type: Optional[str] = None):
    now = datetime.now(IST).strftime("%Y-%m-%d %H:%M:%S")
    thumb = f"/media/thumb?id={media_id}" if media_id else None
    db_exec(
        "INSERT INTO received_messages (Timestamp, Linewalker, Message, Sender, MediaId, MediaType, MediaThumb) VALUES (:ts, :lw, :msg, :snd, :mid, :mt, :thumb)",
        {"ts": now, "lw": linewalker, "msg": message, "snd": user, "mid": media_id, "mt": media_type, "thumb": thumb}
    )
...
async def telegram_webhook(request: Request):
    ...
    media_id_logged: Optional[int] = None
    media_type_logged: Optional[str] = None
    ...
    if text_msg:
        ...
        if is_off:
            ...
            if media_id_logged:
                log_received_message(linewalker=linewalker, message="📷 [image received]", user=user_first,
                                     media_id=media_id_logged, media_type=media_type_logged)
            log_received_message(linewalker=linewalker, message=text_msg, user=user_first)
    elif caption:
        # caption accompanies an image
        if media_id_logged:
    ...
    try:
        with engine.begin() as conn:
            df_sent = pd.read_sql(text("""
                ...
                FROM sent_logs ORDER BY id DESC
            """), conn)
            df_received = pd.read_sql(text("""
                SELECT id, Timestamp, Linewalker, Message, Sender, MediaId, MediaType, MediaThumb
                FROM received_messages ORDER BY id DESC
            """), conn)
...
# ===================== Webhook Management (admin paths for your UI) =====================
...
def refresh_backend(req: RefreshRequest, auth=Depends(verify_api_key)):
    ...
    if req.reset_webhook:
        try:
            result["webhook"] = admin_set_webhook(Request, auth)  # guarded route
