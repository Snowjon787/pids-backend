...
def _table_has_column(table: str, column: str) -> bool:
    try:
        ...
        # If information_schema isn't available (e.g., SQLite), assume column exists
        return True
...
# Build absolute URLs even if PUBLIC_WEBHOOK_URL is not set
def _abs_url(request: Request, path_qs: str) -> str:
    if PUBLIC_WEBHOOK_URL:
        return f"{PUBLIC_WEBHOOK_URL.rstrip('/')}{path_qs}"
...
def get_received_logs(request: Request):
    try:
        cols = _select_list(
            "received_messages", 
            ["id", "Timestamp", "Linewalker", "Message", "Sender", "MediaId", "MediaType", "MediaThumb"]
        ...
        for log in logs:
            ...
            # add absolute links if media present
            if log.get("MediaId"):
                enhanced_log["MediaLink"]    = _abs_url(request, f"/media/thumb?id={log['MediaId']}")
                enhanced_log["DownloadLink"] = _abs_url(request, f"/download_media?id={log['MediaId']}")
            # hide raw columns
...
async def telegram_webhook(request: Request):
    ...
    if text_msg:
        ...
        if is_off:
            ...
            if media_id_logged:
                thumb_url = _abs_url(request, f"/media/thumb?id={media_id_logged}")
