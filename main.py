...
# NEW: convenience wrapper for BOT token
def get_bot_token() -> Optional[str]:
    tok = get_setting("BOT_TOKEN")
...
# ===================== Webhook Management =====================
def _perform_set_webhook() -> Dict[str, Any]:
    token = get_bot_token()
    if not token:
    ...
    try:
        res = SESSION.get(f"https://api.telegram.org/bot{token}/setWebhook",
...
def refresh_backend(req: RefreshRequest):
    ...
    if req.reset_webhook:
        try:
            result["webhook"] = _perform_set_webhook()
...
# ---- Admin-friendly wrappers (for frontend buttons) ----
@app.get("/admin/set_webhook")
def admin_set_webhook(auth: str = Depends(verify_api_key)):
    # Use proper secret variable and unified helper
    try:
        return _perform_set_webhook()
    except HTTPException as e:
...
def admin_get_webhook_info(auth: str = Depends(verify_api_key)):
    token = get_bot_token()
    if not token:
        return {"error": "Bot token not found in DB"}
    res = SESSION.get(
    ...
    try:
        ...
        return {"code": res.status_code, "raw": res.text}
...
def debug_telegram_chat(chat_id: str = Query(...)):
    bot_token = get_bot_token()
    if not bot_token:
        raise HTTPException(status_code=400, detail="BOT_TOKEN not set")
